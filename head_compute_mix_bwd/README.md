# KernelSwift 算子创新大赛 — head_compute_mix_bwd

## 目录结构

`bench/`、`env/`、`run.sh` 是**所有算子共用**的基础设施，放在仓库根目录，
不属于本文件夹；每个算子一个文件夹（本文件夹就是 `head_compute_mix_bwd` 这一
...
└── head_compute_mix_bwd/             本文件夹
    ├── README.md                     本文件
    ├── tasks/
    │   └── head_compute_mix_bwd.py   赛题原始文件（未修改，供对照）
    ├── v0/head_compute_mix_bwd.py    torch 参考实现（Model）—— 加速比的基准
    ├── v1/head_compute_mix_bwd.py    Triton 优化实现（ModelNew）—— 参赛作品
    └── results/                      性能测试结果
```
...
# 2. 跑对拍 + 计时（--only 按 task 名过滤，本算子叫 head_compute_mix_bwd）
bash run.sh --only head_compute_mix_bwd
```
...
|---|---|---:|---:|---:|---|---:|:---:|
| head_compute_mix_bwd | MetaX C500 | 0.2385 | 0.1290 | **1.85x** | 1.57 1.85 1.85 | 15.3% | ✅ 通过 |
...
| head_compute_mix_bwd | MTT S4000 | — | — | — | — | — | ❌ 编译失败 |
