...
@triton.autotune(
    ...
    #              └─  x  ─┘        └─ act ─┘       └─ w1g + w1u + w2t ─┘
    #
    #   这个式子是**保守上界**，在 C500 上校准过 7 个点：
    #     BLOCK_T=64 / ns=2 → 估 122880，triton 报 "Required: 122880" —— 逐字节相符
    #     BLOCK_T=16 / ns=1 → 估  55296，实测 53248（少 2048 = 16·I·2）
    #     BLOCK_T=32 / ns=1 → 估  61440，实测 57344（少 4096 = 32·I·2）
    #   差的正好是 act 那一项：ns=1 时三块权重不双缓冲，w1g 在第一个 dot 之后就死了，
    #   分配器把 act 复用到它的位置上；ns=2 时权重全程活着，act 才要单独占地方。
    #   留着这一项当安全余量，不改式子。
    #
