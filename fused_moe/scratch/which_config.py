...
v1 有 expert + reduce 两个 kernel，config 是叉乘关系，逐 config 全扫没意义，
所以报告各自的赢家 + 端到端耗时 + 分 kernel 计时。
"""
...
def main():
    ...
    with torch.no_grad():
        model(hidden_states, router_logits)   # 触发 autotune 做出选择
    torch.cuda.synchronize()

    tuners = _autotuners(mod)
    print("=" * 72)
    print(f"{len(tuners)} 个挂了 autotune 的 kernel")
    ...
    # config 是叉乘关系，逐 config 全扫没意义；但**分 kernel 计时**很有意义 ——
    # 决定 partial[E,T,H] 那 332 KB 的往返值不值得想办法省掉。
    _per_kernel(mod, model, hidden_states, router_logits, ms)


def _per_kernel(mod, model, hidden_states, router_logits, end_to_end_ms):
