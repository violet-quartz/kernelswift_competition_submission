# KernelSwift 算子创新大赛 — grouped_topk

## 目录结构

`bench/`、`env/`、`run.sh` 是**所有算子共用**的基础设施，放在仓库根目录，
不属于本文件夹；每个算子一个文件夹（本文件夹就是 `grouped_topk` 这一题），
...
└── grouped_topk/                     本文件夹
    ├── README.md                     本文件
    ├── tasks/
    │   └── grouped_topk.py           赛题原始文件（未修改，供对照）
    ├── v0/grouped_topk.py            torch 参考实现（Model）—— 加速比的基准
    ├── v1/grouped_topk.py            Triton 优化实现（ModelNew）—— 参赛作品
    └── results/                      性能测试结果
```
...
# 2. 跑对拍 + 计时（--only 按 task 名过滤，本算子叫 grouped_topk）
bash run.sh --only grouped_topk
```
...
|---|---|---:|---:|---:|---|---:|:---:|
| grouped_topk | MetaX C500 | 0.3604 | 0.1204 | **3.06x** | 2.95 3.06 3.22 | 8.8% | ✅ 通过 |
| grouped_topk | Ascend 910B2C | 0.1742 | 0.1196 | **1.46x** | 1.42 1.46 1.48 | 4.0% | ✅ 通过 |
| grouped_topk | Iluvatar BI-V150 | 0.4631 | 0.1670 | **2.79x** | 2.75 2.79 2.80 | 1.9% | ✅ 通过 |
| grouped_topk | Enflame S60 | 0.4538 | 0.2922 | **1.55x** | 1.55 1.55 1.72 | 11.1% | ✅ 通过 |
| grouped_topk | Hygon BW1000 | 0.3763 | 0.1690 | **2.24x** | 2.21 2.24 2.26 | 2.1% | ✅ 通过 |
| grouped_topk | MTT S4000 | — | — | — | — | — | ❌ 编译失败 |
