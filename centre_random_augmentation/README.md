# KernelSwift 算子创新大赛 — centre_random_augmentation

## 目录结构

`bench/`、`env/`、`run.sh` 是**所有算子共用**的基础设施，放在仓库根目录，
不属于本文件夹；每个算子一个文件夹（本文件夹就是 `centre_random_augmentation` 这一题），
...
└── centre_random_augmentation/       本文件夹
    ├── README.md                     本文件
    ├── tasks/
    │   └── centre_random_augmentation.py 赛题原始文件（未修改，供对照）
    ├── v0/centre_random_augmentation.py torch 参考实现（Model）—— 加速比的基准
    ├── v1/centre_random_augmentation.py Triton 优化实现（ModelNew）—— 参赛作品
    └── results/                      性能测试结果
```
...
# 2. 跑对拍 + 计时（--only 按 task 名过滤，本算子叫 centre_random_augmentation）
bash run.sh --only centre_random_augmentation
```
...
|---|---|---:|---:|---:|---|---:|:---:|
| centre_random_augmentation | MetaX C500 | 0.9840 | 0.1725 | **5.70x** | 5.67 5.70 8.14 | 43.5% | ✅ 通过 |
...
| centre_random_augmentation | MTT S4000 | — | — | — | — | — | ❌ 编译失败 |
