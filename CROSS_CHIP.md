...
| 芯片 | 通过 | 几何平均 | 备注 |
|---|---|---|---|
| 沐曦 MetaX C500 | 10/10 | **3.68x** | ⚠ 早期单次测量，见下方说明 |
...
| Task05 | `music_flamingo_rotary_embedding` | 1.98x | 1.39x | 2.20x | 1.78x | 2.07x | **3.34x** |
| Task06 | `mm_encoder_attention` | 1.05x | 1.46x | 0.63x | 0.74x | 1.06x | ✗ |
| Task07 | `mhc_post` | 12.36x | 2.88x | 16.95x | 0.80x | 7.45x | **10.18x** |
| Task08 | `hc_split_sinkhorn` | 14.75x | 7.81x | 8.35x | 5.11x | 10.01x | **26.24x** |
...
沐曦那一列取自早期的**单次测量**，无逐轮数据，与其余各列（3 轮交替执行取中位数）
不完全可比。该机器在后续验证期间出现驱动层故障：

```
[MXKW][E] mxkwAllocRingBuff: ioctl alloc ringbuffer failed -1
[MCR][E]  Device::acquireQueue: mxc_queue_acquire failed!
```

一个 4×4 矩阵乘即可复现，`mx-smi` 仍报卡空闲可用。故未能在最终代码上复测。

代码层面，沐曦走的是各分支开关的 `else` 路径（判据均为白名单，沐曦不在名单内），
逻辑与历史版本一致。
...
- 各卡噪声水平差异很大（天数 1.2% / 昇腾 5% / 燧原 11%），比较改动优劣时
